...
public enum CheckoutUIControl {
	...
	private class ProceedFromCartToShipBill implements EventHandler<ActionEvent> {
		@Override
		public void handle(ActionEvent evt) {
			...
			shippingBillingWindow.loadDefaultData(CheckoutData.INSTANCE.getDefaultShippingData(),
		...
		public void handle(ActionEvent evt) {
			...
			if (shippingBillingWindow.getSaveShipAddr()) {
				try {
					...
					shipUpdates = runAddressRules(shippingBillingWindow.getShippingAddress());
			...
			if (rulesOk) {
				if (shippingBillingWindow.getSaveBillAddr())
					try {
						...
						billUpdates = runAddressRules(shippingBillingWindow.getBillingAddress());
					} catch (RuleException e) {
						rulesOk = false;
				...
				// if all rules passed
				if (rulesOk) {
					// payment window creates
					paymentWindow = new PaymentWindow();
					paymentWindow.show();
					shippingBillingWindow.hide();
					System.out.println("Rules passed!");
					if (shipUpdates != null) {
						System.out.println("Shipping address updates: " + shipUpdates.toString());
					}
					if (billUpdates != null) {
						System.out.println("Billing address updates: " + shipUpdates.toString());
					}
				}
			}
		}
	...
	private List<String> runPaymentRules(Address addr, CreditCard cc) throws RuleException {
		try {
			// set up
			...
			PaymentBean paymentbean = new PaymentBean(addr, cc);
	...
	public class SaveShipChangeListener implements ChangeListener<Boolean> {
		...
		public void changed(ObservableValue<? extends Boolean> observed, Boolean oldval, Boolean newval) {
			// shippingBillingWindow.displayInfo("Need to write code for
			// this.");
		}
	}
	...
	private class ProceedToTermsHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent evt) {
			...
			try {
				billUpdates = runPaymentRules(shippingBillingWindow.getBillingAddress(),
						paymentWindow.getCreditCardFromWindow());
			} catch (RuleException e) {
				rulesOk = false;
				paymentWindow.displayError("Payment information error: " + e.getMessage());
			}
			...
			if (billUpdates != null) {
				System.out.println("Shipping address updates: " + billUpdates.toString());
			}
